...
(function(){
  ...
  const ACCESSORY_PORTRAIT_IMAGES={glasses:'accessory-glasses-portrait-transparent.png',headband:'accessory-headband-portrait-transparent.png',helmet:'accessory-helmet-portrait-transparent.png',earrings:'accessory-earrings-portrait-transparent.png',scarf:'accessory-scarf-portrait-transparent.png'};
  function characterPortraitSource(style=state.character?.hairStyle,accessories=selectedAccessories()){const accessory=accessories.length===1?accessories[0]:'';return ACCESSORY_PORTRAIT_IMAGES[accessory]||CHARACTER_PORTRAIT_IMAGES[style]||CHARACTER_PORTRAIT_IMAGES.spiky;}
  ...
  function characterMarkup(size='large',override={},presentation={}){
    ...
    const accessories=selectedAccessories(c),portraitAccessory=accessories.length?' portrait-accessory':'',combinedAccessories=accessories.length>1?' combined-accessories':'',accessoryLayers=accessories.length>1?`<div class="portrait-accessory-overlays" aria-hidden="true">${accessories.map(id=>`<i class="portrait-accessory-layer ${id}"></i>`).join('')}</div>`:'';
    return `<div class="miner-avatar ${size}${portraitAccessory}${combinedAccessories}" data-skin="${c.skin}" data-hair-style="${c.hairStyle}" data-hair-color="${c.hairColor}" data-shirt="${c.shirt}" data-pants="${c.pants}" data-accessory="multi" data-jacket="${fashion.jacket}" data-gloves="${fashion.gloves}" data-shoes="${fashion.shoes}" aria-label="Customized miner character">
      <img class="avatar-render" src="${avatarImage}" alt="" draggable="false">${accessoryLayers}<div class="native-wardrobe-layers" aria-hidden="true">${layer('skin')}${layer('hair')}${layer('shirt')}${layer('pants')}${layer('gloves')}${layer('shoes')}</div><div class="avatar-shadow"></div><div class="avatar-legs"><i></i><i></i></div><div class="avatar-body"><div class="avatar-shirt-detail"></div><div class="avatar-collar"></div><div class="avatar-utility-belt"><i></i><i></i></div><div class="avatar-pendant"></div><div class="avatar-arm left"></div><div class="avatar-arm right"></div></div><div class="avatar-neck"></div><div class="avatar-head"><div class="avatar-ear left"></div><div class="avatar-ear right"></div><div class="avatar-hair back"></div><div class="avatar-face"><i class="brow left"></i><i class="brow right"></i><i class="eye left"></i><i class="eye right"></i><i class="mouth"></i></div><div class="avatar-hair front"></div><div class="avatar-hair-streak"></div><div class="avatar-accessory"></div></div></div>`;
  }
  ...
  window.renderJapaneseMinerCharacterShop=function(box){
    ...
    box.querySelectorAll('[data-character-key]').forEach(b=>b.addEventListener('click',()=>{const key=b.dataset.characterKey,value=b.dataset.characterValue;if(!cosmeticOwned(key,value))return;if(key==='accessory')setSelectedAccessories(value==='none'?[]:[value]);else state.character[key]=value;save();render();renderShop();}));
  };
  ...
  function renderFeatureCenter(tab){
    ...
    content.querySelectorAll('[data-character-key]').forEach(b=>b.addEventListener('click',()=>{const key=b.dataset.characterKey,value=b.dataset.characterValue;if(key==='accessory')setSelectedAccessories(value==='none'?[]:[value]);else state.character[key]=value;save();renderFeatureCenter('profile');render();}));
