...
document.addEventListener('click',event=>{
  ...
  if(cosmetic){
    ...
    if(!owned){
      ...
      state.ownedCosmetics.push(id);if(key==='accessory'){const chosen=Array.isArray(state.character.accessories)?state.character.accessories:[];state.character.accessories=value==='none'?[]:[...new Set([...chosen,value])];state.character.accessory=state.character.accessories[0]||'none';}else state.character[key]=value;save();cosmetic.closest('.character-customizer')?.querySelectorAll(`[data-character-key="${key}"]`).forEach(x=>x.classList.toggle('selected',key==='accessory'?state.character.accessories.includes(x.dataset.characterValue):x===cosmetic));cosmetic.classList.remove('locked');cosmetic.classList.add('owned');const priceLabel=cosmetic.querySelector('small');if(priceLabel)priceLabel.textContent='Owned';render();
...
(function(){
  ...
  function ensureV38(){
    ...
    state.character.accessories=Array.isArray(state.character.accessories)?state.character.accessories.filter(id=>validAccessories.includes(id)):validAccessories.includes(legacyAccessory)?[legacyAccessory]:[];
  ...
  function setSelectedAccessories(items){state.character.accessories=[...new Set(items)].filter(id=>['glasses','headband','helmet','earrings','scarf'].includes(id));state.character.accessory=state.character.accessories[0]||'none';}
  ...
  const ACCESSORY_PORTRAIT_IMAGES={glasses:'accessory-glasses-portrait-fixed.png',headband:'accessory-headband-portrait-fixed.png',helmet:'accessory-helmet-portrait-fixed.png',earrings:'accessory-earrings-portrait-fixed.png',scarf:'accessory-scarf-portrait-fixed.png'};
  function characterPortraitSource(style=state.character?.hairStyle,accessories=selectedAccessories()){const accessory=accessories[0];return ACCESSORY_PORTRAIT_IMAGES[accessory]||CHARACTER_PORTRAIT_IMAGES[style]||CHARACTER_PORTRAIT_IMAGES.spiky;}
  ...
  function characterMarkup(size='large',override={},presentation={}){
    ...
    const c=tier<1?Object.assign({},savedCharacter,{hairStyle:'spiky',hairColor:'brown',shirt:'miner',pants:'denim',accessory:'none',accessories:[]}):savedCharacter;
    const avatarImage=characterPortraitSource(c.hairStyle,selectedAccessories(c));
    const maskStyle=CHARACTER_PORTRAIT_IMAGES[c.hairStyle]?c.hairStyle:'spiky';
    const fashion=tier<1?{jacket:'none',gloves:'none',shoes:'boots'}:Object.assign({jacket:'none',gloves:'none',shoes:'boots'},state.v5?.fashion||{});if(presentation.hideJacket)fashion.jacket='none';const sources=renderedSources(c,maskStyle,fashion);
    const layer=(name)=>`<img class="native-cosmetic-layer native-${name}-layer" ${sources[name]?`src="${sources[name]}"`:''} alt="" draggable="false">`;
    const accessories=selectedAccessories(c);
    return `<div class="miner-avatar ${size}" data-skin="${c.skin}" data-hair-style="${c.hairStyle}" data-hair-color="${c.hairColor}" data-shirt="${c.shirt}" data-pants="${c.pants}" data-accessory="multi" data-jacket="${fashion.jacket}" data-gloves="${fashion.gloves}" data-shoes="${fashion.shoes}" aria-label="Customized miner character">
      <img class="avatar-render" src="${avatarImage}" alt="" draggable="false"><div class="native-wardrobe-layers" aria-hidden="true">${layer('skin')}${layer('hair')}${layer('shirt')}${layer('pants')}${layer('gloves')}${layer('shoes')}</div><div class="avatar-shadow"></div><div class="avatar-legs"><i></i><i></i></div><div class="avatar-body"><div class="avatar-shirt-detail"></div><div class="avatar-collar"></div><div class="avatar-utility-belt"><i></i><i></i></div><div class="avatar-pendant"></div><div class="avatar-arm left"></div><div class="avatar-arm right"></div></div><div class="avatar-neck"></div><div class="avatar-head"><div class="avatar-ear left"></div><div class="avatar-ear right"></div><div class="avatar-hair back"></div><div class="avatar-face"><i class="brow left"></i><i class="brow right"></i><i class="eye left"></i><i class="eye right"></i><i class="mouth"></i></div><div class="avatar-hair front"></div><div class="avatar-hair-streak"></div><div class="avatar-accessory"></div></div></div>`;
  }
  ...
  window.renderJapaneseMinerCharacterShop=function(box){
    ...
    box.querySelectorAll('[data-character-key]').forEach(b=>b.addEventListener('click',()=>{const key=b.dataset.characterKey,value=b.dataset.characterValue;if(!cosmeticOwned(key,value))return;if(key==='accessory'){const selected=selectedAccessories();setSelectedAccessories(value==='none'?[]:selected.includes(value)?selected.filter(id=>id!==value):[...selected,value]);}else state.character[key]=value;save();render();renderShop();}));
  };
  ...
  function renderFeatureCenter(tab){
    ...
    content.querySelectorAll('[data-character-key]').forEach(b=>b.addEventListener('click',()=>{const key=b.dataset.characterKey,value=b.dataset.characterValue;if(key==='accessory'){const selected=selectedAccessories();setSelectedAccessories(value==='none'?[]:selected.includes(value)?selected.filter(id=>id!==value):[...selected,value]);}else state.character[key]=value;save();renderFeatureCenter('profile');render();}));
