...
(()=>{
'use strict';
const VERSION='6.4.70-fitted-accessory-restore', SNAPSHOT_PREFIX='jm_v6_snapshots_', FEEDBACK_INBOX_KEY='jm_v6_admin_feedback_inbox', FEEDBACK_INBOX_LIMIT=200;
...
function addMenuItems(){const grid=document.querySelector('.menu-wheel');if(!grid||grid.querySelector('[data-v6-menu]'))return;[['guide','â”','Game Guide','Complete how-to for courses, bosses, goals, economy, and study tools','player'],['settings','âš™ï¸','Accessibility','Text, motion, audio, recovery, and save management','player'],['patreon','â­','Patreon','View supporter tiers and their unlocks','player']].forEach(([id,icon,name,desc,cat])=>{const b=document.createElement('button');b.type='button';b.dataset.v6Menu=id;b.dataset.menuCategoryName=cat;b.innerHTML=`<span>${icon}</span><strong>${name}</strong><small>${desc}</small>`;b.onclick=()=>{closeGameMenu();id==='guide'?openTour():id==='settings'?openSettings():openPatreon();};grid.appendChild(b);});}
