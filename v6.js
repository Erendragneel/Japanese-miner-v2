// Japanese Miner v6.0 â€” The Polish Update
(()=>{
'use strict';
const VERSION='6.4.80-discord-feedback', SNAPSHOT_PREFIX='jm_v6_snapshots_', FEEDBACK_INBOX_KEY='jm_v6_admin_feedback_inbox', FEEDBACK_INBOX_LIMIT=200;
let deferredInstallPrompt=null,installEventsBound=false;
const TOUR=[
 {section:'Getting started',icon:'â›ï¸',title:'Welcome to Japanese Miner',text:'Answer Japanese questions, build mastery, collect scientific gemstones, and progress from Hiragana through JLPT N1.',tips:['Create or sign in to a local player profile so every feature uses the same save.','Tap New Question or the mine rock to begin the selected course route.','Use the round menu button at the bottom-right whenever you need another game area.']},
 ...
 {section:'Goals and rewards',icon:'ðŸ“œ',title:'Complete personal Quests',text:'Quests reward consistent practice through a separate set of daily and weekly goals.',tips:['Open Quests directly from Explore.','Daily goals reset each calendar day; weekly goals reset every Monday.','Rewards include Nuggets plus Hints or Shields. Progress and claimed rewards remain in the current player save.']},
 {section:'Goals and rewards',icon:'ðŸŽ¯',title:'Run Expedition Missions',text:'Missions are daily mine-focused objectives for answering, accuracy, scheduled review, and treasure-chest progress.',tips:['Open Missions directly from Explore or from the Expedition Hub.','The dayâ€™s Nugget values are locked to the active mine when the mission board resets.','Each Sakura Garden level adds 10% to mission Nuggets; supply rewards are listed separately and are not multiplied.']},
 ...
 {section:'Settings and safety',icon:'âš™ï¸',title:'Accessibility, installation, saves, and recovery',text:'Use Accessibility for text, motion, contrast, audio, app installation, explanations, and local safety snapshots.',tips:['Install App or Add to Home Screen is available inside Accessibility & Settings when supported by the device.','Safety snapshots provide local recovery points; restoring one replaces the current saved state.','Use the visible Back buttons to return to the parent menu, and avoid clearing browser storage before exporting a backup.']}
];
function ensureV6(){
 state.v6=Object.assign({tourComplete:false,onboardingGuideOffered:false,textSize:'normal',reducedMotion:false,highContrast:false,colorAssist:false,sfxVolume:70,musicVolume:35,explanations:true,feedback:[],storySeen:[],lastSnapshot:0,supporterTier:0},state.v6||{});
...
function supporterTier(){ensureV6();return isDeveloperSession===true?3:(Number(state.v6.supporterTier)||0);}
function supporterGateMarkup(tier,feature){const label=['','Supporter','Companion Keeper','Settlement Founder'][tier]||'Supporter';return `<section class="supporter-gate" data-supporter-tier="${tier}"><span>ðŸ”’ PATREON TIER ${tier}</span><h3>${esc6(feature)} is for ${label}s</h3><p>Connect your Patreon account when membership linking launches to unlock this section.</p><strong>Required tier: ${label}</strong></section>`;}
...
function openFeedback(){ensureV6();openOverlay('feedback');const dialog=document.getElementById('v6FeedbackOverlay'),box=document.getElementById('v6FeedbackContent');if(dialog){const kicker=dialog.querySelector('.menu-header-copy span'),title=dialog.querySelector('.menu-header-copy h2');if(kicker)kicker.textContent='Help improve the mine';if(title)title.textContent='💬 Player Feedback';}if(isDeveloperSession)renderAdminFeedbackInbox(box);else renderPlayerFeedback(box);box?.insertAdjacentHTML('afterbegin','<section class="v6-discord-feedback"><span>COMMUNITY DISCORD</span><h3>Join the Japanese Miner community</h3><p>Share ideas, report issues, and talk with other players on Discord.</p><a class="supporter-join-link" href="https://discord.gg/AGrGMFjXQm" target="_blank" rel="noopener">Join our Discord</a></section>');}
function openPatreon(){ensureV6();openOverlay('feedback');const dialog=document.getElementById('v6FeedbackOverlay'),box=document.getElementById('v6FeedbackContent');dialog?.querySelector('.menu-header-copy span')&&(dialog.querySelector('.menu-header-copy span').textContent='Japanese Miner Support');dialog?.querySelector('.menu-header-copy h2')&&(dialog.querySelector('.menu-header-copy h2').textContent='â­ Patreon Tiers');if(box)box.innerHTML=`<section class="supporter-tier-list"><article class="supporter-tier tier-one"><span>ðŸŸ© TIER 1 Â· SUPPORTER</span><h3>Cosmetics and community</h3><p>Clothing, hats, accessories, pickaxe skins, character titles, wallpapers, Discord role, behind-the-scenes posts, development updates, poll voting, and optional Supporter credits.</p></article><article class="supporter-tier tier-two"><span>ðŸŸ¦ TIER 2 Â· COMPANION KEEPER</span><h3>Everything in Tier 1, plus companions</h3><p>Companion system, companion cosmetics, companion names, companion idle animations, and the exclusive companion collection.</p></article><article class="supporter-tier tier-three"><span>ðŸŸ¨ TIER 3 Â· SETTLEMENT FOUNDER</span><h3>Everything above, plus settlements</h3><p>Settlement system, decorations, exclusive buildings, seasonal decorations, and settlement customization.</p></article><p class="supporter-link-note">Membership linking is coming later. Until then, locked sections show the required tier.</p></section>`;}
function addMenuItems(){const grid=document.querySelector('.menu-wheel');if(!grid||grid.querySelector('[data-v6-menu]'))return;[['guide','❔','Game Guide','Complete how-to for courses, bosses, goals, economy, and study tools','player'],['feedback','💬','Feedback','Join the Discord community and share feedback','player'],['settings','⚙️','Accessibility','Text, motion, audio, recovery, and save management','player'],['patreon','⭐','Patreon','View supporter tiers and their unlocks','player']].forEach(([id,icon,name,desc,cat])=>{const b=document.createElement('button');b.type='button';b.dataset.v6Menu=id;b.dataset.menuCategoryName=cat;b.innerHTML=`<span>${icon}</span><strong>${name}</strong><small>${desc}</small>`;b.onclick=()=>{closeGameMenu();id==='guide'?openTour():id==='feedback'?openFeedback():id==='settings'?openSettings():openPatreon();};grid.appendChild(b);});}
...
function openPatreon(){ensureV6();openOverlay('feedback');const dialog=document.getElementById('v6FeedbackOverlay'),box=document.getElementById('v6FeedbackContent');if(dialog){const kicker=dialog.querySelector('.menu-header-copy span'),title=dialog.querySelector('.menu-header-copy h2');if(kicker)kicker.textContent='Japanese Miner Support';if(title)title.textContent='â­ Patreon Tiers';}if(box)box.innerHTML=`<section class="supporter-tier-list"><article class="supporter-tier tier-one"><img src="patreon-tier-1-supporter.png" alt="Supporter tier cover: $1 per month"><span>ðŸŸ© TIER 1 Â· SUPPORTER Â· $1 / MONTH</span><h3>Cosmetics and community</h3><p>Clothing, hats, accessories, pickaxe skins, character titles, wallpapers, Discord role, behind-the-scenes posts, development updates, poll voting, and optional Supporter credits.</p></article><article class="supporter-tier tier-two"><img src="patreon-tier-2-companion-keeper.png" alt="Companion Keeper tier cover: $3 per month"><span>ðŸŸ¦ TIER 2 Â· COMPANION KEEPER Â· $3 / MONTH</span><h3>Everything in Tier 1, plus companions</h3><p>Companion system, companion cosmetics, companion names, companion idle animations, and the exclusive companion collection.</p></article><article class="supporter-tier tier-three"><img src="patreon-tier-3-settlement-founder.png" alt="Settlement Founder tier cover: $5 per month"><span>ðŸŸ¨ TIER 3 Â· SETTLEMENT FOUNDER Â· $5 / MONTH</span><h3>Everything above, plus settlements</h3><p>Settlement system, decorations, exclusive buildings, seasonal decorations, and settlement customization.</p></article><p class="supporter-link-note">Membership linking is coming later. Until then, locked sections show the required tier.</p></section>`;}
const originalOpenPatreon=openPatreon;openPatreon=function(){originalOpenPatreon();document.getElementById('v6FeedbackContent')?.insertAdjacentHTML('beforeend','<a class="supporter-join-link" href="https://www.patreon.com/cw/Erendragneel/membership" target="_blank" rel="noopener">Join Japanese Miner on Patreon</a>');};
